...
def main(args):
    ...
    if args.train_data_dir is not None:
        ...
        if len(cache_list)>0:
            ...
            del tokenizer_one, text_encoder_one
            gc.collect()
            ...
            with accelerator.accumulate(unet):
                with accelerator.autocast():
                    ...
                    noise_scheduler.set_timesteps(
                        max_denoising_steps, device=accelerator.device
                    )
                    optimizer.zero_grad()
                    # 1 ~ 49 からランダム
                    ...
                    noise = randn_tensor(latents.shape, generator=generator, device=accelerator.device)
                    ...
                    noisy_model_input = noisy_model_input.to(accelerator.device, dtype=weight_dtype)
                    noise = noise.to(accelerator.device, dtype=weight_dtype)
                    ...
                    # training_timesteps = max_denoising_steps
                    torch.set_grad_enabled(True)

                    # 7. Denoising loop
                    # num_warmup_steps = len(timesteps) - training_timesteps * noise_scheduler.order
                    timesteps = noise_scheduler.timesteps
                    training_timesteps = range(len(noise_scheduler.timesteps))
                    for i, t in tqdm(enumerate(timesteps)):
                        # torch.set_grad_enabled((len(training_timesteps) == 0 or i > min(training_timesteps)))
                        # expand the latents if we are doing classifier free guidance
                        noisy_model_input = noise_scheduler.scale_model_input(noisy_model_input, t)
                    ...
                    out = noise_scheduler.step(noise_pred, t, latents, generator, return_dict=True)
                    latents = out.prev_sample
                    # image = vae.decode(latents.to(vae.dtype) / vae.config.scaling_factor, return_dict=False)[0]
